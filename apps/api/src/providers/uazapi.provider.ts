...
export class UazapiProvider implements WhatsAppProvider {
  ...
  private async resolveCredentials(): Promise<{ baseUrl: string; token: string }> {
  ...
  async sendMessage(to: string, message: string): Promise<SendResult> {
        const { baseUrl, token } = await this.resolveCredentials();
  ...
  async sendMedia(to: string, url: string, mediaType: MediaType, caption?: string): Promise<SendResult> {
      ...
      // UazAPI v2 uses a single unified /send/media endpoint with a "type" field
      ...
      if (caption) body.caption = caption;
        if (mediaType === "document") body.docName = url.split("/").pop() ?? "media";
